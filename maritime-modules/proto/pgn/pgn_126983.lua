...
local alertSystem = ProtoField.uint8("nmea-2000-126983.alertSystem", "Alert System")
local alertSubSystem = ProtoField.uint8("nmea-2000-126983.alertSubSystem", "Alert Sub-System")
local alertId = ProtoField.uint16("nmea-2000-126983.alertId", "Alert ID")
local dataSourceNetworkIdName = ProtoField.uint64("nmea-2000-126983.dataSourceNetworkIdName", "Data Source Network ID NAME")
local dataSourceInstance = ProtoField.uint8("nmea-2000-126983.dataSourceInstance", "Data Source Instance")
local dataSourceIndexSource = ProtoField.uint8("nmea-2000-126983.dataSourceIndexSource", "Data Source Index-Source")
local alertOccurrenceNumber = ProtoField.uint8("nmea-2000-126983.alertOccurrenceNumber", "Alert Occurrence Number")
...
local acknowledgeSourceNetworkIdName = ProtoField.uint64("nmea-2000-126983.acknowledgeSourceNetworkIdName", "Acknowledge Source Network ID NAME")
...
local alertPriority = ProtoField.uint8("nmea-2000-126983.alertPriority", "Alert Priority")
...
function NMEA_2000_126983.dissector(buffer, pinfo, tree)
    ...
    if buffer:len() >= (str_offset + 17 + 8) then
        subtree:add_le(acknowledgeSourceNetworkIdName, buffer(str_offset + 17, 8))
    end
    ...
    if buffer:len() >= (str_offset + 26 + 1) then
        subtree:add(alertPriority, buffer(str_offset + 26, 1))
    end
