...
local sid = ProtoField.uint8("nmea-2000-129284.sid", "SID")
...
local etaDate = ProtoField.uint16("nmea-2000-129284.etaDate", "ETA Date (d)")
...
local originWaypointNumber = ProtoField.uint32("nmea-2000-129284.originWaypointNumber", "Origin Waypoint Number")
local destinationWaypointNumber = ProtoField.uint32("nmea-2000-129284.destinationWaypointNumber", "Destination Waypoint Number")
...
function NMEA_2000_129284.dissector(buffer, pinfo, tree)
    ...
    if buffer:len() >= (str_offset + 10 + 2) then
        subtree:add_le(etaDate, buffer(str_offset + 10, 2))
    end
    ...
    if buffer:len() >= (str_offset + 16 + 4) then
        subtree:add_le(originWaypointNumber, buffer(str_offset + 16, 4))
    end
    if buffer:len() >= (str_offset + 20 + 4) then
        subtree:add_le(destinationWaypointNumber, buffer(str_offset + 20, 4))
    end
