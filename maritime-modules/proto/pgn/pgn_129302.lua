...
local sid = ProtoField.uint8("nmea-2000-129302.sid", "SID")
...
local originMarkId = ProtoField.uint32("nmea-2000-129302.originMarkId", "Origin Mark ID")
local destinationMarkId = ProtoField.uint32("nmea-2000-129302.destinationMarkId", "Destination Mark ID")
...
function NMEA_2000_129302.dissector(buffer, pinfo, tree)
    ...
    if buffer:len() >= (str_offset + 9 + 4) then
        subtree:add_le(originMarkId, buffer(str_offset + 9, 4))
    end
    if buffer:len() >= (str_offset + 13 + 4) then
        subtree:add_le(destinationMarkId, buffer(str_offset + 13, 4))
    end
end
