...
local alertSystem = ProtoField.uint8("nmea-2000-126985.alertSystem", "Alert System")
local alertSubSystem = ProtoField.uint8("nmea-2000-126985.alertSubSystem", "Alert Sub-System")
local alertId = ProtoField.uint16("nmea-2000-126985.alertId", "Alert ID")
local dataSourceNetworkIdName = ProtoField.uint64("nmea-2000-126985.dataSourceNetworkIdName", "Data Source Network ID NAME")
local dataSourceInstance = ProtoField.uint8("nmea-2000-126985.dataSourceInstance", "Data Source Instance")
local dataSourceIndexSource = ProtoField.uint8("nmea-2000-126985.dataSourceIndexSource", "Data Source Index-Source")
local alertOccurrenceNumber = ProtoField.uint8("nmea-2000-126985.alertOccurrenceNumber", "Alert Occurrence Number")
...
function NMEA_2000_126985.dissector(buffer, pinfo, tree)
    ...
    if buffer:len() >= (str_offset + 17 + 1) then
        ...
        if length and length >= 0 and buffer:len() >= (str_offset + 17 + 2 + length) then
            ...
            str_offset = str_offset + 17 + length + 2
        end
    end
    if buffer:len() >= (str_offset + 1) then
        ...
        if length and length >= 0 and buffer:len() >= (str_offset + 2 + length) then
            ...
            subtree:add(alertLocationTextDescription, buffer(str_offset + 2, length))
