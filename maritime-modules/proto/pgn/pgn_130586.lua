...
local volumeLimit = ProtoField.uint8("nmea-2000-130586.volumeLimit", "Volume limit (%)")
local fade = ProtoField.int8("nmea-2000-130586.fade", "Fade (%)")
local balance = ProtoField.int8("nmea-2000-130586.balance", "Balance (%)")
local subVolume = ProtoField.uint8("nmea-2000-130586.subVolume", "Sub volume (%)")
local eqTreble = ProtoField.int8("nmea-2000-130586.eqTreble", "EQ - Treble (%)")
local eqMidRange = ProtoField.int8("nmea-2000-130586.eqMidRange", "EQ - Mid range (%)")
local eqBass = ProtoField.int8("nmea-2000-130586.eqBass", "EQ - Bass (%)")
...
local highPassFilterFrequency = ProtoField.uint16("nmea-2000-130586.highPassFilterFrequency", "High pass filter frequency (Hz)")
local lowPassFilterFrequency = ProtoField.uint16("nmea-2000-130586.lowPassFilterFrequency", "Low pass filter frequency (Hz)")
...
function NMEA_2000_130586.dissector(buffer, pinfo, tree)
    ...
    if buffer:len() >= (str_offset + 1 + 1) then
        subtree:add(volumeLimit, buffer(str_offset + 1, 1))
    end
    if buffer:len() >= (str_offset + 2 + 1) then
        subtree:add(fade, buffer(str_offset + 2, 1))
    end
    if buffer:len() >= (str_offset + 3 + 1) then
        subtree:add(balance, buffer(str_offset + 3, 1))
    end
    if buffer:len() >= (str_offset + 4 + 1) then
        subtree:add(subVolume, buffer(str_offset + 4, 1))
    end
    if buffer:len() >= (str_offset + 5 + 1) then
        subtree:add(eqTreble, buffer(str_offset + 5, 1))
    end
    if buffer:len() >= (str_offset + 6 + 1) then
        subtree:add(eqMidRange, buffer(str_offset + 6, 1))
    end
    if buffer:len() >= (str_offset + 7 + 1) then
        subtree:add(eqBass, buffer(str_offset + 7, 1))
    end
    ...
    if buffer:len() >= (str_offset + 10 + 2) then
        subtree:add_le(highPassFilterFrequency, buffer(str_offset + 10, 2))
    end
    if buffer:len() >= (str_offset + 12 + 2) then
        subtree:add_le(lowPassFilterFrequency, buffer(str_offset + 12, 2))
    end
