...
local userId = ProtoField.uint32("nmea-2000-129793.userId", "User ID")
...
local communicationState = ProtoField.uint32("nmea-2000-129793.communicationState", "Communication State", base.HEX)
...
local positionDate = ProtoField.uint16("nmea-2000-129793.positionDate", "Position Date (d)")
local gnssType = ProtoField.uint8("nmea-2000-129793.gnssType", "GNSS type", base.DEC, NULL, 0xf0)

NMEA_2000_129793.fields = {messageId,repeatIndicator,userId,longitude,latitude,positionAccuracy,raim,positionTime,communicationState,aisTransceiverInformation,positionDate,gnssType}
...
function NMEA_2000_129793.dissector(buffer, pinfo, tree)
    ...
    if buffer:len() >= (str_offset + 21 + 2) then
        subtree:add_le(positionDate, buffer(str_offset + 21, 2))
    end
