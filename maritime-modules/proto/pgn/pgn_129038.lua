...
local userId = ProtoField.uint32("nmea-2000-129038.userId", "User ID")
...
local communicationState = ProtoField.uint32("nmea-2000-129038.communicationState", "Communication State", base.HEX)
...
local sequenceId = ProtoField.uint8("nmea-2000-129038.sequenceId", "Sequence ID")

NMEA_2000_129038.fields = {messageId,repeatIndicator,userId,longitude,latitude,positionAccuracy,raim,timeStamp,cog,sog,communicationState,aisTransceiverInformation,heading,rateOfTurn,navStatus,specialManeuverIndicator,sequenceId}
...
function NMEA_2000_129038.dissector(buffer, pinfo, tree)
    ...
    if buffer:len() >= (str_offset + 27 + 1) then
        subtree:add(sequenceId, buffer(str_offset + 27, 1))
    end
end
