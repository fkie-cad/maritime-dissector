...
local userId = ProtoField.uint32("nmea-2000-129041.userId", "User ID")
...
local atonStatus = ProtoField.bytes("nmea-2000-129041.atonStatus", "AtoN Status")
...
NMEA_2000_129041.fields = {messageId,repeatIndicator,userId,longitude,latitude,positionAccuracy,raim,timeStamp,lengthDiameter,beamDiameter,positionReferenceFromStarboardEdge,positionReferenceFromTrueNorthFacingEdge,atonType,offPositionIndicator,virtualAtonFlag,assignedModeFlag,positionFixingDeviceType,atonStatus,aisTransceiverInformation,atonName}
...
function NMEA_2000_129041.dissector(buffer, pinfo, tree)
    ...
    if buffer:len() >= (str_offset + 24 + 1) then
        subtree:add(atonStatus, buffer(str_offset + 24, 1))
    end
