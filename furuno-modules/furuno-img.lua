...
function FURUNO_IMG.dissector(buffer, pinfo, tree)
    ...
    local spoke_cnt_val = bit.rshift(buffer(9, 1):uint(), 1)
    subtree:add_le(spoke_cnt, buffer(9, 1))                 --          XXXXXXX.

    local cell_cnt_val = bit.band(buffer(10, 2):le_uint(), 0x7ff)
    subtree:add_le(cell_cnt, buffer(10, 2))                 -- XXXXXXXX .....XXX
    local compr_lvl_val = bit.rshift(bit.band(buffer(11, 1):le_uint(), 0x18), 3)
    ...
    elseif compr_lvl_val == 3 then
        ...
        do
            ...
            do
                ...
                byte_cnt = byte_cnt + 1
                if bit.band(byte, 3) > 0 then
                    -- reference or RLE
                    cnt = cnt + bit.rshift(byte, 2)
                else
                    -- no encoding
                    cnt = cnt + 1
                end
