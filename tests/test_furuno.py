...
def test_furuno_img():
    process = subprocess.run(
        [
            ...
            "json",
            "-X",
            f"lua_script:{script_dir}/../furuno-dissector.lua",
            "-r",
            f"{script_dir}/furuno-radar-img.pcap",
            "-o",
            "furuno-img.split_spokes:false",
            "-2",
        ],
    ...
    assert (
        len(pkt["furuno-img.azimuth_list"].split("  "))
        == len(pkt["furuno-img.heading_list"].split("  "))
        == int(pkt["furuno-img.spoke_cnt"])
    )
