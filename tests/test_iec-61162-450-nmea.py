...
FRAME_PROTOCOLS = [
    "eth:ethertype:ip:udp:iec-61162-450-nmea:iec-61162-450-nmea:nmea-0183",
    "eth:ethertype:ip:udp:nmea-0183:nmea-0183",
    "eth:ethertype:ip:udp:iec-61162-450-nmea:iec-61162-450-nmea-multi:iec-61162-450-nmea:nmea-0183:iec-61162-450-nmea:nmea-0183:iec-61162-450-nmea:nmea-0183:iec-61162-450-nmea:nmea-0183:iec-61162-450-nmea:nmea-0183:iec-61162-450-nmea:nmea-0183:iec-61162-450-nmea:nmea-0183:iec-61162-450-nmea:nmea-0183",
]
LAYERS = ["iec-61162-450-nmea-multi", "iec-61162-450-nmea", "nmea-0183"]
...
def packet_nmea_0183():
    process = subprocess.run(
        [
            ...
            "json",
            "-X",
            f"lua_script:{script_dir}/../maritime-dissector.lua",
            "-r",
            f"{script_dir}/nmea-0183.pcap",
        ],
...
def packet_nmea_0183_wrong_checksum():
    process = subprocess.run(
        [
            ...
            f"lua_script:{script_dir}/../maritime-dissector.lua",
            "-r",
            f"{script_dir}/nmea-0183-wrong-chcksm.pcap",
        ],
        stdout=subprocess.PIPE,
    )
    packet = json.loads(process.stdout, object_pairs_hook=duplicate_keys_to_array)
...
def packet_iec_450_nmea():
    process = subprocess.run(
        [
            ...
            f"lua_script:{script_dir}/../maritime-dissector.lua",
            "-r",
            f"{script_dir}/iec-61162-450-nmea.pcap",
        ],
        stdout=subprocess.PIPE,
    )
    packet = json.loads(process.stdout, object_pairs_hook=duplicate_keys_to_array)
...
def packet_iec_450_nmea_multitag():
    process = subprocess.run(
        [
            ...
            f"lua_script:{script_dir}/../maritime-dissector.lua",
            "-r",
            f"{script_dir}/iec-61162-450-nmea-multitag.pcap",
        ],
        stdout=subprocess.PIPE,
    )
    packet = json.loads(process.stdout, object_pairs_hook=duplicate_keys_to_array)
...
@pytest.fixture
def packet_iec_450_nmea_multisentence():
    process = subprocess.run(
        [
            "tshark",
            "-T",
            ...
            f"lua_script:{script_dir}/../maritime-dissector.lua",
            "-r",
            f"{script_dir}/iec-61162-450-nmea-multisentence.pcap",
        ],
        stdout=subprocess.PIPE,
    )
    packet = json.loads(process.stdout, object_pairs_hook=duplicate_keys_to_array)
...
def packet_iec_450_nmea_wrong_tag_checksum():
    process = subprocess.run(
        [
            ...
            f"lua_script:{script_dir}/../maritime-dissector.lua",
            "-r",
            f"{script_dir}/iec-61162-450-nmea-wrong-tag-chcksm.pcap",
        ],
        stdout=subprocess.PIPE,
    )
    packet = json.loads(process.stdout, object_pairs_hook=duplicate_keys_to_array)
...
def packet_nmea_0183_sentence_too_long():
    process = subprocess.run(
        [
            ...
            f"lua_script:{script_dir}/../maritime-dissector.lua",
            "-r",
            f"{script_dir}/nmea-0183-sentence-too-long.pcap",
        ],
        stdout=subprocess.PIPE,
    )
    packet = json.loads(process.stdout, object_pairs_hook=duplicate_keys_to_array)
...
def packet_iec_450_nmea_sentence_too_long():
    process = subprocess.run(
        [
            ...
            f"lua_script:{script_dir}/../maritime-dissector.lua",
            "-r",
            f"{script_dir}/iec-61162-450-nmea-sentence-too-long.pcap",
        ],
        stdout=subprocess.PIPE,
    )
    packet = json.loads(process.stdout, object_pairs_hook=duplicate_keys_to_array)
...
def packet_iec_450_nmea_tag_too_long():
    process = subprocess.run(
        [
            ...
            f"lua_script:{script_dir}/../maritime-dissector.lua",
            "-r",
            f"{script_dir}/iec-61162-450-nmea-tag-too-long.pcap",
        ],
        stdout=subprocess.PIPE,
    )
    packet = json.loads(process.stdout, object_pairs_hook=duplicate_keys_to_array)
...
def packets_all(
    packet_nmea_0183,
    packet_nmea_0183_wrong_checksum,
    packet_iec_450_nmea,
    packet_iec_450_nmea_multitag,
    packet_iec_450_nmea_multisentence,
    packet_iec_450_nmea_wrong_tag_checksum,
):
    return (
        packet_nmea_0183,
        packet_nmea_0183_wrong_checksum,
...
def test_frame_protocols_iec_450_nmea(packets_all):
    for packet in packets_all:
        assert (
            packet["_source"]["layers"]["frame"]["frame.protocols"] in FRAME_PROTOCOLS
        )
...
def test_iec_450_nmea(packet_iec_450_nmea):
    ...
    assert (
        "iec-61162-450-nmea.token"
        in packet_iec_450_nmea["_source"]["layers"]["iec-61162-450-nmea"]
    )
    assert (
...
def test_iec_450_nmea_token(packet_iec_450_nmea):
    assert (
        "UdPbC"
        == packet_iec_450_nmea["_source"]["layers"]["iec-61162-450-nmea"][
            "iec-61162-450-nmea.token"
        ]
    )
...
def test_iec_450_nmea_0183(packet_iec_450_nmea):
    assert (
        "nmea-0183.talkerid"
        in packet_iec_450_nmea["_source"]["layers"]["iec-61162-450-nmea"]["nmea-0183"]
    )
    assert (
        "HE"
        == packet_iec_450_nmea["_source"]["layers"]["iec-61162-450-nmea"]["nmea-0183"][
            "nmea-0183.talkerid"
        ]
    )
    assert (
        "nmea-0183.sentenceid"
        in packet_iec_450_nmea["_source"]["layers"]["iec-61162-450-nmea"]["nmea-0183"]
    )
    assert (
        "ROT"
...
def test_nmea_0183_talkerid(packet_nmea_0183):
    assert (
        "HE" == packet_nmea_0183["_source"]["layers"]["nmea-0183"]["nmea-0183.talkerid"]
    )
...
def test_nmea_0183_sentenceid(packet_nmea_0183):
    assert (
        "ROT"
        == packet_nmea_0183["_source"]["layers"]["nmea-0183"]["nmea-0183.sentenceid"]
    )
...
def test_nmea_0183_data(packet_nmea_0183):
    assert (
        "+000.05,A"
        == packet_nmea_0183["_source"]["layers"]["nmea-0183"]["nmea-0183.data"]
    )
...
def test_nmea_0183_checksum(packet_nmea_0183):
    assert (
        "0x35 [valid]"
        == packet_nmea_0183["_source"]["layers"]["nmea-0183"]["nmea-0183.checksum"]
    )
...
def test_iec_450_nmea_multitag(packet_iec_450_nmea_multitag):
    ...
    assert (
        "iec-61162-450-nmea.token"
        in packet_iec_450_nmea_multitag["_source"]["layers"]["iec-61162-450-nmea"]
    )
    assert (
        "iec-61162-450-nmea.tags"
        in packet_iec_450_nmea_multitag["_source"]["layers"]["iec-61162-450-nmea"]
...
def test_iec_450_nmea_multitag_token(packet_iec_450_nmea_multitag):
    assert (
        "UdPbC"
        == packet_iec_450_nmea_multitag["_source"]["layers"]["iec-61162-450-nmea"][
            "iec-61162-450-nmea.token"
        ]
    )
...
@pytest.mark.dependency(depends=["test_iec_450_nmea"])
def test_iec_450_nmea_multisentence(packet_iec_450_nmea_multisentence):
    talkerids = 8 * ["IN"]
    sentenceids = ["GLL", "HCR", "ROT", "VBC", "VBW", "VTG", "VDR", "OSD"]
    tag_ns = ["n:881", "n:882", "n:883", "n:884", "n:885", "n:886", "n:887", "n:888"]
    for i, sentence in enumerate(
        packet_iec_450_nmea_multisentence["_source"]["layers"][
            "iec-61162-450-nmea-multi"
        ]["iec-61162-450-nmea"]
    ):
        assert talkerids[i] == sentence["nmea-0183"]["nmea-0183.talkerid"]
        assert sentenceids[i] == sentence["nmea-0183"]["nmea-0183.sentenceid"]
        assert tag_ns[i] in sentence["iec-61162-450-nmea.tags"]


@pytest.mark.dependency(depends=["test_iec_450_nmea"])
def test_iec_450_nmea_wrong_tag_checksum(
    packet_iec_450_nmea, packet_iec_450_nmea_wrong_tag_checksum
):
    ws_expert = {
    ...
    assert (
        "_ws.expert"
        in packet_iec_450_nmea_wrong_tag_checksum["_source"]["layers"][
            "iec-61162-450-nmea"
        ]
    )
    assert (
        packet_iec_450_nmea_wrong_tag_checksum["_source"]["layers"][
            "iec-61162-450-nmea"
        ]["_ws.expert"]
        == ws_expert
    )
...
def test_nmea_0183_wrong_checksum(packet_nmea_0183_wrong_checksum):
    ...
    assert (
        "_ws.expert"
        in packet_nmea_0183_wrong_checksum["_source"]["layers"]["nmea-0183"]
    )
    assert (
        packet_nmea_0183_wrong_checksum["_source"]["layers"]["nmea-0183"]["_ws.expert"]
        == ws_expert
    )
...
def test_nmea_0183_sentence_too_long(packet_nmea_0183_sentence_too_long):
    ...
    assert (
        "_ws.expert"
        in packet_nmea_0183_sentence_too_long["_source"]["layers"]["nmea-0183"]
    )
    assert (
        packet_nmea_0183_sentence_too_long["_source"]["layers"]["nmea-0183"][
            "_ws.expert"
        ]
        == ws_expert
    )
...
def test_iec_450_nmea_sentence_too_long(packet_iec_450_nmea_sentence_too_long):
    ...
    assert (
        "_ws.expert"
        in packet_iec_450_nmea_sentence_too_long["_source"]["layers"][
            "iec-61162-450-nmea"
        ]["nmea-0183"]
    )
    assert (
        packet_iec_450_nmea_sentence_too_long["_source"]["layers"][
            "iec-61162-450-nmea"
        ]["nmea-0183"]["_ws.expert"]
        == ws_expert
    )
...
def test_iec_450_nmea_tag_too_long(packet_iec_450_nmea_tag_too_long):
    ...
    assert (
        "_ws.expert"
        in packet_iec_450_nmea_tag_too_long["_source"]["layers"]["iec-61162-450-nmea"]
    )
    assert (
        packet_iec_450_nmea_tag_too_long["_source"]["layers"]["iec-61162-450-nmea"][
            "_ws.expert"
        ]
        == ws_expert
    )
